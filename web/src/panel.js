 ...
 * Performs automated local syncing every 60 minutes.
 */
...
client.once('ready', () => {
  ...
  // Set clean custom presence status
  client.user.setActivity('World Cup Virtual Pool', { type: ActivityType.Competing });

  // Automated 60-Minute Local Sync & Pinned Panel Refresh loop
  setInterval(async () => {
  ...
  }, 1000 * 60 * 60); // Exact hourly interval (60 minutes)
});
